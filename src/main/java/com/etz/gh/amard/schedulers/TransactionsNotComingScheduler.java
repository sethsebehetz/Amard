...
import com.etz.gh.amard.jobs.TransactionsNotComingJob;
 ...
 * @author seth.sebeh
 */
public class TransactionsNotComingScheduler extends SchedulerFactory implements Runnable{

    Scheduler scheduler;
    List<Monitor> serviceMonitors;

    public TransactionsNotComingScheduler(List<Monitor> serviceMonitors, String name) throws SchedulerException {
    ...
    public void run() {
        try {
            System.out.println("STARTING REQUEST NOT COMING JOB PROCESSOR");
            for (Monitor service : this.serviceMonitors) {
                JobDetail jobDetail = JobBuilder.newJob(TransactionsNotComingJob.class).withIdentity(service.getType()+ "::" +service.getName(), service.getGroup()).build();
            ...
            System.out.println("EXCEPTION IN REQUEST NOT COMING PROCESSER >> " + ex);
        }
    }
}
